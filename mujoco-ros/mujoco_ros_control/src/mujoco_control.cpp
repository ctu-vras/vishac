...
int main(int argc, char **argv) {
    ...
    ros::Publisher pub_pc = node.advertise<PointCloud>("points", 1);
    ...
    const auto timer_rendered = node.createTimer(ros::Duration(0.1), [&](const ros::TimerEvent &e) {
        ...
        sensor_msgs::CameraInfo cam_info = renderer.GetCamInfo(depth);
        ...
        if (pub_rgb.getNumSubscribers() != 0) {
            header.frame_id = "virtual_camera";
            pub_rgb.publish(cv_bridge::CvImage(header, "rgb8", rgb).toImageMsg());
        }

        if (pub_depth.getNumSubscribers() != 0) {
            header.frame_id = "virtual_camera";
            pub_depth.publish(cv_bridge::CvImage(header, "32FC1", depth).toImageMsg());
        }
